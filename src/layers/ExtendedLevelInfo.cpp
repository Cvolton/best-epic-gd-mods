...
bool ExtendedLevelInfo::init(){
    ...
    auto descText = BetterInfo::fixNullByteCrash(BetterInfo::fixColorCrashes(level->getUnpackedLevelDescription()));
...
void ExtendedLevelInfo::showProgressDialog(GJGameLevel* level){
    ...
    PaginatedFLAlert::create(level->levelName, contents)->show();
}